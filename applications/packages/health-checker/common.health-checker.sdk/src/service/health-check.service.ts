...
export class HealthCheckService {
    ...
    constructor(
        ...
        this._client = new HealthCheckClient(baseUrl);
    }

    public async checkHealth(
    ): Promise<OperationResult<HCHealthResultV1Model>> {
        const result = await this._client
            .health(EXTERNAL_API_ENDPOINTS.v1.health.forClient(this._token));
        if (isNil(result)) {
    ...
    public async checkMetrics(
        ...
        const result = await this._client
            .metrics(EXTERNAL_API_ENDPOINTS.v1.metrics.forClient(this._token));
        if (isNil(result)) {
